...
```typescript
--8<-- "user-guide/concepts/model-providers/amazon-bedrock.ts:request_timeout_handler_options"
```
...
In Python, that provider can also target `bedrock-runtime`; see
[Choosing the Endpoint](#choosing-the-endpoint).
...
The config targets `bedrock-mantle` by default. In Python, set `endpoint` to
`bedrock-runtime`, which is where the OpenAI GPT models are served through
[cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html)
profile ids such as `global.openai.gpt-5.6-luna`:
...
AWS compares what the two endpoints support in
