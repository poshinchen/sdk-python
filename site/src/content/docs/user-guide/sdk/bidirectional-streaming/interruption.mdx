...
When a user starts speaking while the model is still responding, `BidiAgent` detects it and stops the current response, so conversations feel natural instead of forcing the user to wait their turn.
